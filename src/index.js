...
app.get("/meme/:id", (req, res) => {
  ...
  try {
    meme = fs.readdirSync(path.join(__dirname, "public", "memes")).find(meme => meme.split(".")[0] === id)
    image = fs.readFileSync(path.join(__dirname, "public", "memes", meme))
    exetension = meme.split(".")[1]
  }
  catch (err) {
...
app.get("/media/:id", (req, res) => {
  ...
  try {
    media = fs.readdirSync(path.join(__dirname, "public", "media")).find(media => media.split(".")[0] === id)
    image = fs.readFileSync(path.join(__dirname, "public", "media", media))
    exetension = media.split(".")[1]
  }
  catch (err) {
    return res.status(404).json({
...
app.get("/image/:id", (req, res) => {
  ...
  try {
    image = fs.readFileSync(path.join(__dirname, "public", "images", id))
  }
  catch (err) {
    return res.status(404).json({
...
app.listen(port, () => {
  ...
  // TODO: FIX: startup.checkForInvalidFiles({ verbose: false });
